...
export class ContentService {
  ...
  async create(createContentDto: CreateContentDto, userId: string): Promise<Content> {
    try {
      ...
      await this.invalidateContentCaches(content.type)
  ...
  async findAll(
    ...
    try {
      await this.redisService.set(cacheKey, JSON.stringify(result), 1800) // 30 minutes
    } catch (error) {
  ...
  async findPublic(type?: ContentType): Promise<Content[]> {
    const cacheKey = type
      ? this.redisService.generateKey("content", "public", type)
      : this.redisService.generateKey("content", "public", "all")
    ...
    const content = await this.contentModel.find(query).sort({ publishedAt: -1 }).exec()

    try {
      await this.redisService.set(cacheKey, JSON.stringify(content), 3600) // 1 hour
    } catch (error) {
      console.error("Redis set error:", error)
    }
  ...
  async findByType(type: ContentType): Promise<Content | null> {
    const cacheKey = this.redisService.generateKey("content", "type", type)
  ...
  async findOne(id: string): Promise<Content> {
    ...
    const cacheKey = this.redisService.generateKey("content", id)
  ...
  async update(id: string, updateContentDto: UpdateContentDto, userId: string): Promise<Content> {
    ...
    await this.invalidateContentCaches(updatedContent.type, id)
  ...
  async remove(id: string): Promise<void> {
    ...
    await this.invalidateContentCaches(content.type, id)
  }
  ...
  private async invalidateContentCaches(type?: ContentType, id?: string): Promise<void> {
    try {
      const keysToDelete: string[] = []

      // Clear specific content cache
      if (id) {
        keysToDelete.push(this.redisService.generateKey("content", id))
      }

      // Clear type-specific cache
      if (type) {
        keysToDelete.push(this.redisService.generateKey("content", "type", type))
      }

      // Clear public content caches
      keysToDelete.push(
        this.redisService.generateKey("content", "public", "all"),
        ...(type ? [this.redisService.generateKey("content", "public", type)] : []),
      )
      ...
      // For paginated results, we'll need to clear all cached pages
      // Since we can't use pattern deletion, we'll rely on TTL expiration
