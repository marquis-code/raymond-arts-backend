...
export class ReviewService {
  ...
  async createReview(createReviewDto: CreateProductReviewDto): Promise<ProductReview> {
    try {
      ...
      let productNameForReview: string | undefined = createReviewDto.productName

      // Determine if review is linked to a product ID or has a custom name
      if (createReviewDto.productId) {
        ...
        productExists = true
        productNameForReview = product.name // Use product's actual name if linked
      } else if (!createReviewDto.productName) {
        // This case should ideally be caught by DTO validation, but as a fallback
        throw new BadRequestException("Either productId or productName must be provided.")
      }

      // Check for duplicate review by email for the specific product/productName
      let existingReviewQuery: any = { email: createReviewDto.email.toLowerCase() }
      if (productExists) {
        existingReviewQuery.productId = new Types.ObjectId(createReviewDto.productId)
      } else {
        existingReviewQuery.productName = productNameForReview
      }

      const existingReview = await this.reviewModel.findOne(existingReviewQuery)

      if (existingReview) {
        throw new BadRequestException("This email has already reviewed this product/item.")
      }

      // Create review with all required fields
      const reviewData: Partial<ProductReview> = {
        productId: productExists ? new Types.ObjectId(createReviewDto.productId!) : undefined,
        productName: productNameForReview, // Store the product name (either from product or custom)
        userId: undefined, // No user ID for anonymous reviews
        userName: createReviewDto.userName || "Anonymous",
        email: createReviewDto.email.toLowerCase(),
        userRole: UserRole.ANONYMOUS,
  ...
  async updateReviewStatus(reviewId: string, status: ProductReviewStatus, userId: string): Promise<ProductReview> {
    ...
    // Only update product rating if the review is linked to a product ID
    if (updatedReview.productId && (status === ProductReviewStatus.APPROVED || previousStatus === ProductReviewStatus.APPROVED)) {
      await this.updateProductRating(updatedReview.productId.toString())
    }
    return updatedReview
  }
  ...
  async findAllReviews(
    ...
    const skip = (page - 1) * limit
    const [reviews, total] = await Promise.all([
      this.reviewModel.find().sort({ createdAt: -1 }).skip(skip).limit(limit).populate("productId", "name images price").exec(),
      this.reviewModel.countDocuments(),
    ])
    return {
      reviews,
  ...
  async findByProduct(
    ...
    const [reviews, total, stats] = await Promise.all([
      this.reviewModel.find(filter).sort({ createdAt: -1 }).skip(skip).limit(limit).populate("productId", "name").exec(),
      this.reviewModel.countDocuments(filter),
      this.getReviewStats(productId),
    ])
    return {
  ...
  async findPendingReviews(
    ...
    const skip = (page - 1) * limit
    const [reviews, total] = await Promise.all([
      this.reviewModel.find({ status: ProductReviewStatus.PENDING }).sort({ createdAt: -1 }).skip(skip).limit(limit).populate("productId", "name images").exec(),
      this.reviewModel.countDocuments({ status: ProductReviewStatus.PENDING }),
    ])
    return {
      reviews,
  ...
  async findUserReviews(
    ...
    const skip = (page - 1) * limit
    const [reviews, total] = await Promise.all([
      this.reviewModel.find({ userId }).sort({ createdAt: -1 }).skip(skip).limit(limit).populate("productId", "name images price").exec(),
      this.reviewModel.countDocuments({ userId }),
    ])
    return {
      reviews,
  ...
  async approveReview(reviewId: string, approvedBy: string): Promise<ProductReview> {
    ...
    if (updatedReview.productId) {
      await this.updateProductRating(updatedReview.productId.toString())
      await this.productService.incrementApprovedReviews(updatedReview.productId.toString())
    }
    return updatedReview
  }
  ...
  async rejectReview(reviewId: string, rejectionReason: string): Promise<ProductReview> {
    ...
    // If review was previously approved AND linked to a product, update product stats
    if (wasApproved && updatedReview.productId) {
      await this.updateProductRating(updatedReview.productId.toString())
      await this.productService.decrementApprovedReviews(updatedReview.productId.toString())
    }
    return updatedReview
  }
  ...
  async deleteReview(reviewId: string, userId: string): Promise<void> {
    ...
    const wasApproved = review.status === ProductReviewStatus.APPROVED
    await this.reviewModel.findByIdAndDelete(reviewId)
    // Update product stats if review was approved AND linked to a product
    if (wasApproved && review.productId) {
      await this.updateProductRating(review.productId.toString())
      await this.productService.decrementApprovedReviews(review.productId.toString())
    }
  }
