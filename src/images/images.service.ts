...
export class ImagesService {
  ...
  // Main upload method - converts to WebP, reduces size, and saves to DB
  async uploadFile(file: Express.Multer.File, createImageDto?: CreateImageDto): Promise<ImageDocument> {
    try {
      ...
      // Always convert to WebP with size reduction
      const uploadResult = await this.uploadFileAsWebP(file);
      ...
      const imageData = {
        ...
        description: createImageDto?.description || '',
        // tags: this.parseTags(createImageDto?.tags),
  ...
  // Direct WebP upload (existing method, now also saves to DB if called directly)
  async uploadFileAsWebP(file: Express.Multer.File, createImageDto?: CreateImageDto): Promise<any> {
    try {
      const originalSize = file.size;
      console.log(`🎯 FORCING size reduction for: ${file.originalname}`);
      console.log(`Original file size: ${originalSize} bytes (${(originalSize / 1024).toFixed(1)} KB)`);
      ...
      try {
        ...
        // If createImageDto is provided, also save to database
        if (createImageDto) {
          const nameWithoutExt = file.originalname.split('.').slice(0, -1).join('.');
          const imageData = {
            name: createImageDto.name || nameWithoutExt,
  ...
  // Extreme compression method (existing)
  async uploadFileWithExtremeCompression(file: Express.Multer.File, targetSizeKB?: number, createImageDto?: CreateImageDto): Promise<any> {
    try {
      const originalSize = file.size;
      const targetSize = targetSizeKB ? targetSizeKB * 1024 : Math.floor(originalSize * 0.3); // 70% reduction by default
  ...
  async update(id: string, updateImageDto: UpdateImageDto): Promise<ImageDocument> {
    const updatedImage = await this.imageModel
      .findByIdAndUpdate(
        ...
        {
          ...updateImageDto,
          // tags: updateImageDto.tags ? this.parseTags(updateImageDto.tags) : undefined
        },
        { new: true }
      )
      .exec();
  ...
  async getOptimizedImageUrls(id: string): Promise<any> {
    ...
    return {
      original: image.url,
      thumbnail: `${baseUrl}w_200,h_200,c_fill/${imagePath}`,
      small: `${baseUrl}w_400,h_400,c_fit/${imagePath}`,
      medium: `${baseUrl}w_800,h_800,c_fit/${imagePath}`,
      large: `${baseUrl}w_1200,h_1200,c_fit/${imagePath}`,
    };
  }
  ...
  // Keep all existing private methods for compression
  private async extremeCompression(inputBuffer: Buffer, targetSize: number): Promise<Buffer> {
    console.log(`🔥 Applying extreme compression - target: ${targetSize} bytes`);
