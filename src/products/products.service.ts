...
import { CreateCategoryDto } from "./dto/create-category.dto"
import { UpdateCategoryDto } from "./dto/update-category.dto"
import { ReorderProductItemDto } from "./dto/reorder-product.dto" // Import the new DTO
import { PaginationParams, PaginatedResult } from "../common/interfaces/pagination.interface"
import { CloudinaryService } from "../cloudinary/cloudinary.service"
...
export class ProductsService {
  ...
  //   const maxPositionProduct = await this.productModel.findOne().sort({ position: -1 }).exec()
  //   const newPosition = maxPositionProduct ? maxPositionProduct.position + 1 : 0
  ...
  // async findAllProducts(params: PaginationParams): Promise<PaginatedResult<Product>> {
  //   const { page = 1, limit = 10, sort = "position", order = "asc", search } = params // Default sort by position
  //   const skip = (page - 1) * limit
  ...
  //     throw new BadRequestException(`Invalid product ID: ${id}`)
  //   }
  //   const product = await this.productModel.findById(id).populate("category") // Populate the category field.exec()
  //   if (!product) {
  //     throw new NotFoundException(`Product with ID ${id} not found`)
  //   }
  //   return product
  ...
  // async updateProductOrder(reorderData: ReorderProductItemDto[]): Promise<void> {
  ...
  async getFeaturedProducts(): Promise<Product[]> {
    this.logger.debug("Fetching featured products from database")
    return await this.productModel.find({ featured: true }).exec()
  }
  ...
  async findOne(id: string): Promise<Product> {
    ...
    const cacheKey = this.redisService.generateKey("products", "id", id)
  ...
  async updateRating(productId: string, averageRating: number, reviewCount: number): Promise<void> {
    try {
      ...
      await this.redisService.del(this.redisService.generateKey("products", "id", productId))
      await this.redisService.del(this.redisService.generateKey("products", "all"))
      await this.redisService.del(this.redisService.generateKey("products", "featured"))
    } catch (error) {
      console.error(`Failed to update product rating for ${productId}:`, error)
      throw new BadRequestException("Failed to update product rating")
    }
  ...
  async findOneWithReviewStats(id: string): Promise<Product> {
    if (!Types.ObjectId.isValid(id)) {
      throw new BadRequestException("Invalid product ID")
    }

    const cacheKey = this.redisService.generateKey("products", "reviews", id)
    ...
    return await this.redisService.getOrSet(
      ...
      900, // Cache for 15 minutes (shorter due to review stats)
    )
  }
  ...
  async incrementApprovedReviews(productId: string): Promise<void> {
    ...
    await this.redisService.del(this.redisService.generateKey("products", "id", productId))
    await this.redisService.del(this.redisService.generateKey("products", "reviews", productId))
  }
  ...
  async createProduct(createProductDto: CreateProductDto, userId: string): Promise<Product> {
    ...
    try {
      ...
      await this.redisService.del(this.redisService.generateKey("products", "all"))
      await this.redisService.del(this.redisService.generateKey("products", "featured"))
      await this.redisService.del(this.redisService.generateKey("products", "new"))
      await this.redisService.del(this.redisService.generateKey("products", "bestseller"))
      if (productData.category) {
        await this.redisService.del(
          this.redisService.generateKey("products", "category", productData.category.toString()),
        )
      }
  ...
  async updateProduct(id: string, updateProductDto: UpdateProductDto, userId: string): Promise<Product> {
    ...
    await this.redisService.del(this.redisService.generateKey("products", "id", id))
    await this.redisService.del(this.redisService.generateKey("products", "reviews", id))
    await this.redisService.del(this.redisService.generateKey("products", "all"))
    await this.redisService.del(this.redisService.generateKey("products", "featured"))
    await this.redisService.del(this.redisService.generateKey("products", "new"))
    await this.redisService.del(this.redisService.generateKey("products", "bestseller"))
    if (product.category) {
      await this.redisService.del(this.redisService.generateKey("products", "category", product.category.toString()))
    }
    if (updateProductDto.category) {
      await this.redisService.del(
        this.redisService.generateKey("products", "category", updateProductDto.category.toString()),
      )
    }
  ...
  async findAllProducts(params: PaginationParams): Promise<PaginatedResult<Product>> {
    const { page = 1, limit = 10, sort = "position", order = "asc", search } = params
    const skip = (page - 1) * limit

    const cacheKey = this.redisService.generateKey(
      "products",
      "all",
      `${page}-${limit}-${sort}-${order}-${search || "no-search"}`,
    )
  ...
  async findFeaturedProducts(limit = 8): Promise<Product[]> {
    const cacheKey = this.redisService.generateKey("products", "featured", limit.toString())
  ...
  async findNewProducts(limit = 8): Promise<Product[]> {
    const cacheKey = this.redisService.generateKey("products", "new", limit.toString())
  ...
  async findBestsellerProducts(limit = 8): Promise<Product[]> {
    const cacheKey = this.redisService.generateKey("products", "bestseller", limit.toString())
  ...
  private async getAllChildCategories(
    categoryId: string,
  ): Promise<(Document<unknown, {}, any> & Required<{ _id: unknown }> & { __v: number })[]> {
    const children = await this.categoryModel.find({ parent: categoryId }).exec()
    let allChildren: (Document<unknown, {}, any> & Required<{ _id: unknown }> & { __v: number })[] = [...children]

    for (const child of children) {
      const grandChildren = await this.getAllChildCategories(child._id.toString())
  ...
  async searchProducts(query: string, params: PaginationParams): Promise<PaginatedResult<Product>> {
    ...
    const cacheKey = this.redisService.generateKey("products", "search", query, `${page}-${limit}`)
  ...
  async createCategory(createCategoryDto: CreateCategoryDto, userId: string): Promise<any> {
    console.log("Service received DTO:", createCategoryDto)
    try {
      const categoryData = {
        ...createCategoryDto,
      ...
      await this.redisService.del(this.redisService.generateKey("categories", "all"))
      await this.redisService.del(this.redisService.generateKey("categories", "tree"))
  ...
  async findAllCategories(params: PaginationParams): Promise<PaginatedResult<any>> {
    const { page = 1, limit = 10, sort = "order", order = "asc", search } = params
    const skip = (page - 1) * limit
  ...
  async findCategoryById(id: string): Promise<any> {
    const cacheKey = this.redisService.generateKey("categories", "id", id)

    return await this.redisService.getOrSet(
      cacheKey,
      async () => {
        const category = await this.categoryModel.findById(id).populate("parent", "name").populate("children").exec()
        if (!category) {
          throw new NotFoundException(`Category with ID ${id} not found`)
        }
        return category
  ...
  async updateCategory(id: string, updateCategoryDto: UpdateCategoryDto, userId: string): Promise<any> {
    const category = await this.categoryModel.findById(id).exec()
    if (!category) {
      throw new NotFoundException(`Category with ID ${id} not found`)
    }
    ...
    await this.redisService.del(this.redisService.generateKey("categories", "id", id))
    await this.redisService.del(this.redisService.generateKey("categories", "all"))
    await this.redisService.del(this.redisService.generateKey("categories", "tree"))
    if (updateCategoryDto.parent) {
      await this.redisService.del(this.redisService.generateKey("categories", "id", updateCategoryDto.parent))
    }
  ...
  async removeCategory(id: string, userId: string): Promise<any> {
    const category = await this.categoryModel.findById(id).exec()
    if (!category) {
      throw new NotFoundException(`Category with ID ${id} not found`)
    }
  ...
  async uploadCategoryImage(id: string, file: Express.Multer.File, userId: string): Promise<any> {
    const category = await this.categoryModel.findById(id).exec()
    if (!category) {
      throw new NotFoundException(`Category with ID ${id} not found`)
    }
  ...
  async getCategoryTree(): Promise<any[]> {
    const cacheKey = this.redisService.generateKey("categories", "tree")
